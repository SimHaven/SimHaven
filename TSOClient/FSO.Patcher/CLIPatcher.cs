...
namespace FSO.Patcher
{
    public class CLIPatcher
    {
        ...
        {
            Console.WriteLine("Could not update SimHaven as write access could not be gained to the game files. Try running update.exe as an administrator.");
            Cleanup();
            Environment.Exit(0);
        }
        ...
        private async Task<int> ShowErrors(List<string> remaining)
        {
            ...
            Console.WriteLine("Couldn't write one or more files. Make sure you are not running an instance of SimHaven! \r\nFiles:\r\n\r\n" + fileList);
            return 0;
        }
        ...
        {
            if (!File.Exists("SimHaven.exe")) File.Copy("SimHaven.exe.old", "SimHaven.exe", true);
            if (Environment.OSVersion.Platform == PlatformID.Unix || Environment.OSVersion.Platform == PlatformID.MacOSX)
            {
                Console.WriteLine($"===== Starting SimHaven... Please wait! =====");
                var args = string.Join(" ", Args);
                if (args.Length > 0) args = " " + args;
                var startArgs = new ProcessStartInfo("mono", "SimHaven.exe" + args);
        ...
        public void Begin()
        {
            Console.WriteLine("===== SimHaven Patcher CLI =====");
            Console.WriteLine(Path.Count + " update(s) to apply.");

            if (Args.Contains("--client"))
            {
                Console.WriteLine("SimHaven client requested. Downloading the client package.");
                ToDownload.Add("https://fso-builds.riperiperi.workers.dev/");
            }

            if (Args.Contains("--extras"))
            {
                Console.WriteLine("Unix Extras requested. Downloading the compatibility package.");
