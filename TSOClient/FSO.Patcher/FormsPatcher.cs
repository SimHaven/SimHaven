    ...
    public partial class FormsPatcher : Form
    {
        ...
        private async Task AdvanceExtract()
        {
            if (PathProgress >= Path.Count)
            {
                ...
                Invoke(new Action(() => StartSimHaven()));
            }
            else
            {
                //extract next zip
        ...
        private async Task<int> ShowErrors(List<string> remaining)
        {
            ...
            Invoke(new Action(() => {
                ...
                var dresult = MessageBox.Show("Couldn't write one or more files. Make sure you are not running an instance of SimHaven! \r\nFiles:\r\n\r\n" + fileList,
        ...
        public void StartSimHaven()
        {
            try
            {
                ...
                {
                    var startArgs = new ProcessStartInfo("mono", "SimHaven.exe " + string.Join(" ", Args));
        ...
        private void EmergencyDownload()
        {
            var result = MessageBox.Show("You've started the patcher without any updates queued. If you wish to update the game, log into the game server and it will tell you what to do.\r\n\r\n" +
                "If you opened this application because SimHaven is unopenable, you can attempt to redownload a neutral client version using this application. Do you want to reinstall SimHaven this way?",
